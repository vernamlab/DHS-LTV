...
//////////////////////////////////////////////////////////////////////////////////////////////
ntru::ntru(int dm, int num, int max_bit, int tableS, int wordSize){
	word 	= wordSize;
	ZZ qq 	= to_ZZ("2");
	ZZ BB 	= to_ZZ("1");
	Set(gp, qq, BB, dm);
...
ntru::ntru(ZZ my_p, ZZ my_B, int my_N, int dm, int num, int max_bit, int tableS, int wordSize){
	word 	= wordSize;
	p = my_p;	B = my_B;
	N = my_N;	degree_m = dm;
	rand_seed 	 = 0;
...
ZZX ntru::Prim_Encrypt(ZZX m, int i){
...
ZZX ntru::Prim_Decrypt(ZZX c, int index){
...
ZZX ntru::Prim_RelinRingFFT(ZZX &c, int index, int tableIndx){
...
ZZX ntru::Prim_RelinRingFFT(ZZX &c, int index){
	return Prim_RelinRingFFT(c, index, 0);
}


ZZX ntru::Prim_ModSwitch(ZZX &x, int index){
	ZZX res;
	ZZ tp;
	for(int i=0; i<N; i++){
		tp = coeff(x, i);
...
ZZX ntru::Prim_ModSwitch_LongPoly(ZZX &x, int PolyDeg, int index){
	ZZX res;
	ZZ tp;
...
void ntru::Prim_ReduceKeysLevel(int level){
	Prim_ReduceKeysLevel(level, 0);
}

void ntru::Prim_ReduceKeysLevel(int level, int tblIndex){
...
void ntru::Func_ModulusFindRing(int num, int max_bit, int diff, ZZX modu){
...
ZZX ntru::Func_CreateMessage(int size){
	ZZX x;
...
void ntru::Func_SetModulus(){
	modulus = ComputeFastCycModulus(degree_m);
}

void ntru::Func_ComputeKeysRingRelinFFT(int num, int tblsize){
	tableSize = tblsize;
...
void ntru::Arith_PolyReduce(ZZX &out, ZZX &in){
	myr.Reduce(out, in);
}

void ntru::Arith_CoeffReduce(ZZX &out, ZZX &in, int index){
	int l = in.rep.length();
	coeff_reduction(out, in, q_list[index], l);
}

void ntru::Arith_PolyCoeffReduce(ZZX &out, ZZX &in, int index){
	Arith_PolyReduce(out, in);
	Arith_CoeffReduce(out, out, index);
}

ZZX ntru::Arith_MulModZZX(ZZX &a, ZZX &b, int index){
	ZZX t;
	t = a*b;
	Arith_PolyReduce(t, t);
	coeff_reduction(t, t, q_list[index], N);
...
ZZX ntru::Arith_AddModZZX(ZZX &a, ZZX &b, int index){
	ZZX t = (a+b);
	Arith_PolyReduce(t, t);
	coeff_reduction(t, t, q_list[index], N);
	return t;
...
ZZ *ntru::Pointer_Q(int i){
	return &(q_list[i]);
}

ZZX	*ntru::Pointer_PolyModulus(){
	return (&modulus);
}

GlobalParam *ntru::Pointer_GP(){
	return (&gp);
}

ZZX	*ntru::Pointer_PublicKey(){
	return &(pk[0]);
}

ZZX	*ntru::Pointer_SecretKey(int i){
	return &(sk[i]);
}

ZZX	*ntru::Pointer_EvalKey(int i){
	return &(ek2[0].key[i]);
}
...
void ntru::IO_ReadModulus(fstream &fs){
	fs >> modulus;
	if(reducsetflag == false)
		SetFastPolyModulus();
}

void ntru::IO_ReadQs(fstream &fs, int s){
	for(int i=0; i<s; i++)
		fs >> q_list[i];
}

void ntru::IO_ReadKeys(fstream &fs, int s){
	fs >> pk[0];
	for(int i=0; i<s; i++)
		fs >> sk[i];
...
void ntru::GetPolyWordsIndex(ZZX &bits, ZZX &cip, int &bitIndex){
